...
const FormationDetails = () => {
  ...
  const submitHandler = async (e) => {
    ...
    if(imgUrl){formData.append("imgUrl","http://localhost:5000/uploads/"+imgUrl.name);}
    ...
    try {
      const { data } = await axios.put(`http://localhost:5000/api/formations/formation/${userId}/${singleCarItem._id}`, formData, {
        headers: {
...
useEffect(() => {
  ...
  const fetchData = async () => {
    ...
    try {
      const result = await axios.get(`http://localhost:5000/api/formations/${slug}`);
